...
export class MapAdapter implements IMapAdapter {
  ...
  easeTo(options: EaseToOptions): this {
    ...
    if (options.duration !== undefined) easeOptions.duration = options.duration;
    if (options.pitch !== undefined) easeOptions.pitch = options.pitch;
    if (options.bearing !== undefined) easeOptions.bearing = options.bearing;
  ...
  getStyle(): MapStyle | null {
    ...
    return {
      layers: style.layers.map((layer: { id: string; type: string }) => ({
        id: layer.id,
        type: layer.type,
      })),
    };
